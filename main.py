...
path = '/home/ankama/MEDIGATE/code/'
...
valid_ids = image_ids[-665:] # 665 images for valid
train_ids = image_ids[:-665] # 2 708 images for training
...
for epoch in range(num_epochs):
    ...
    for i in range(len(train_data_loader.dataset)):
        image = [train_data_loader.dataset[i][0]]
        target = [train_data_loader.dataset[i][1]]
...
## Display prediction of first image
...
for box in boxes :
    sample = cv2.rectangle(sample, (box[0], box[1]), (box[2], box[3]), (220, 0, 0), 2)
